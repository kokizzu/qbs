...
func NewMemsql() Dialect {
	d := new(memsql)
	d.base.dialect = d
	return d
}

func DefaultMemSQLDataSourceName(dbName string) *DataSourceName {
	dsn := new(DataSourceName)
	dsn.Dialect = new(memsql)
	dsn.Username = "root"
	dsn.DbName = dbName
...
func (d memsql) parseBool(value reflect.Value) bool {
	return value.Int() != 0
}

func (d memsql) sqlType(field modelField) string {
	f := field.value
		...
		default:
			if len(field.colType) != 0 {
				...
				case QBS_COLTYPE_TEXT:
					...
					return "longtext"
				default:
					panic("Qbs doesn't support column type " + field.colType + " for MemSQL")
				}
			}
		}
	}
...
func (d memsql) indexExists(mg *Migration, tableName, indexName string) bool {
	var row *sql.Row
	var name string
...
func (d memsql) primaryKeySql(isString bool, size int) string {
	if isString {
